...
export default function EnterpriseChat() {
  ...
  const handleDeleteFile = async () => {
    ...
    try {
      const res = await fetch(`/api/delete-file?filename=${encodeURIComponent(target)}`, {
        method: 'DELETE',
      });
      ...
      if (res.ok) {
        setMessages((prev) => [
          ...prev,
          { role: 'system', content: `SYSTEM: "${target}" removed from memory.` },
        ]);
        fetchFiles();
      }
    } catch (err) {
  ...
  if (!isLoaded) {
    return <div className="h-screen bg-[#0B0F1A] flex items-center justify-center text-blue-500 font-mono">LOADING_SESSION...</div>;
  }
